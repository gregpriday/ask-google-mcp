...
// Legacy alias map. "pro" was removed as a tier, but callers (older agents, hardcoded configs)
// may still pass model:"pro". Rather than reject them, we silently normalize "pro" → "flash"
// before validation so those requests keep working. Not advertised in the tool schema.
export const LEGACY_MODEL_ALIASES = Object.freeze({ pro: "flash" });
...
// back to "flash"/first-enabled, preserving the pre-router behavior).
...
// Order from most-to-least capable, used when snapping a requested/fallback model down to an
// enabled tier. Removing "pro" left flash as the top tier.
const MODEL_PREFERENCE_ORDER = Object.freeze(["flash", "flash-lite"]);
...
// Default is "flash" — the safer choice when we genuinely don't know the query's shape.
// flash-lite is cheaper but can be shallow on harder queries; flash is the reliable default.
export const ROUTER_FALLBACK_MODEL = (() => {
  ...
  if (!ENABLED_MODELS.includes(candidate)) {
    const snapped =
      MODEL_PREFERENCE_ORDER.find((m) => ENABLED_MODELS.includes(m)) || ENABLED_MODELS[0];
    console.error(
...
// ASK_GOOGLE_ENABLED_MODELS=flash (flash-lite disabled), the router can't run and we fall back to
// pre-router behavior.
...
export const DEFAULT_MODEL = (() => {
  ...
  if (ENABLED_MODELS.includes("flash")) return "flash";
  const fallback = ENABLED_MODELS[0];
  console.error(
    `[CONFIG] ASK_GOOGLE_ENABLED_MODELS excludes "flash" and router unavailable; default model falls back to "${fallback}"`
  );
  return fallback;
})();
...
// model so we return something rather than failing outright. Defaults to "flash" — the more
// capable of the two remaining tiers.
...
// Gemini 3 `thinkingConfig.thinkingLevel`. Uppercase strings: MINIMAL, LOW, MEDIUM, HIGH.
// Anything else is ignored with a warning so a bad env doesn't take the server down and we
// fall back to the per-model default below.
const VALID_THINKING_LEVELS = new Set(["MINIMAL", "LOW", "MEDIUM", "HIGH"]);

// Per-model default thinking level when no env var is set. Flash/flash-lite stay at the SDK
// default (undefined → SDK picks a light thinking level) since they already lean tool-heavy.
